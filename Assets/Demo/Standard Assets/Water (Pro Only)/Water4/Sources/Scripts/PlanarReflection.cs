...
public class PlanarReflection : MonoBehaviour
{
    ...
    private Dictionary<UnityEngine.Camera, bool> helperCameras = null;
    ...
    private UnityEngine.Camera CreateReflectionCameraFor(UnityEngine.Camera cam)
    {
        System.String reflName = gameObject.name + "Reflection" + cam.name;
        GameObject go = GameObject.Find(reflName);

        if (!go)
            go = new GameObject(reflName, typeof(UnityEngine.Camera));
        if (!go.GetComponent<UnityEngine.Camera>())
            go.AddComponent<UnityEngine.Camera>();
        UnityEngine.Camera reflectCamera = go.GetComponent<UnityEngine.Camera>();
    ...
    private void SetStandardCameraParameter(UnityEngine.Camera cam, LayerMask mask)
    {
    ...
    private RenderTexture CreateTextureFor(UnityEngine.Camera cam)
    {
    ...
    public void RenderHelpCameras(UnityEngine.Camera currentCam)
    {
        if (null == helperCameras)
            helperCameras = new Dictionary<UnityEngine.Camera, bool>();
    ...
    public void WaterTileBeingRendered(Transform tr, UnityEngine.Camera currentCam)
    {
    ...
    private void RenderReflectionFor(UnityEngine.Camera cam, UnityEngine.Camera reflectCamera)
    {
        if (!reflectCamera)
            return;
    ...
    private void SaneCameraSettings(UnityEngine.Camera helperCam)
    {
